...
G4VPhysicalVolume* DRsimDetectorConstruction::Construct() {
  ...
  // ###############################
  // ? NOTE - fModuleDepth - Depth of  the moduel in mm dimension
  ...
  fiberUnit   = new G4Box("fiber_SQ", (fFiberUnitH/2) *mm, (1./2) *mm, (fModuleDepth/2) *mm);
  fiberClad   = new G4Tubs("fiber",  0, clad_C_rMax, fModuleDepth/2., 0 *deg, 360. *deg);   // S is the same
  fiberCoreC  = new G4Tubs("fiberC", 0, core_C_rMax, fModuleDepth/2., 0 *deg, 360. *deg);
  fiberCoreS  = new G4Tubs("fiberS", 0, core_S_rMax, fModuleDepth/2., 0 *deg, 360. *deg);

  dimCalc = new dimensionCalc();
  dimCalc->SetFrontL(fFrontL);
  dimCalc->SetTower_height(fModuleDepth);
...
void DRsimDetectorConstruction::ModuleBuild(G4LogicalVolume* ModuleLogical_[], 
  ...
  for (int i = 0; i < fNofModules; i++) {    
    ...
    module = new G4Box("Mudule", (fModuleH/2.) *mm, (fModuleW/2.) *mm, (fModuleDepth/2.) *mm );
...
void DRsimDetectorConstruction::FiberImplement(G4int i, G4LogicalVolume* ModuleLogical__[], 
  ...
  for (int k = 0; k < NofPlate; k++) {
    for (int j = 0; j < NofFiber; j++) { 
      ...
      */
      G4float fX = -400.*mm/2 + k*2.5*mm + 1.25*mm;
      G4float fY = -400.*mm/2 + j*2.5*mm + 1.25*mm;
